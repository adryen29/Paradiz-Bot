...
const { Client, GatewayIntentBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, ComponentType } = require('discord.js');
...
// --- NOUVEAU : Endpoint de vérification de santé (Anti-down) ---
...
app.post('/request-exchange', async (req, res) => {
    ...
    try {
        ...
        addLog("INFO", `Demande d'échange reçue pour ${username} : ${amount} points`);

        const messages = await logChannel.messages.fetch({ limit: 100 });
        const existingMsg = messages.find(m => {
            try { return JSON.parse(m.content).userId === userId; } catch { return false; }
        });

        if (existingMsg) {
            const data = JSON.parse(existingMsg.content);
            data.amount += amount;
            data.date = new Date().toLocaleString("fr-FR");
            await existingMsg.edit(JSON.stringify(data));
        } else {
            const data = { userId, username, amount, date: new Date().toLocaleString("fr-FR") };
            await logChannel.send(JSON.stringify(data));
        }

        updatePublicTable();
        res.status(200).json({ success: true });
    } catch (error) {
        addLog("ERROR", `Erreur échange pour ${username} : ${error.message}`);
...
async function updatePublicTable() {
    try {
        ...
        const embed = new EmbedBuilder()
            .setTitle("📦 File des paiements en attente")
        ...
        const actionRow = components.length > 0 ? new ActionRowBuilder().addComponents(components) : null;
        const messages = await displayChannel.messages.fetch({ limit: 10 });
        const botMsg = messages.find(m => m.author.id === client.user.id);

        const payload = { embeds: [embed], components: actionRow ? [actionRow] : [] };
        if (botMsg) await botMsg.edit(payload); else await displayChannel.send(payload);
    } catch (e) {
        console.error("Erreur lors de la mise à jour du tableau :", e);
    }
}
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.customId.startsWith('delete_')) {
        ...
        const confirmRow = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`confirm_${userId}`).setLabel("Confirmer le paiement").setStyle(ButtonStyle.Success),
            new ButtonBuilder().setCustomId("cancel").setLabel("Annuler").setStyle(ButtonStyle.Secondary)
        );

        await interaction.reply({ content: "⚠️ Confirmer la suppression (le joueur a été payé) ?", components: [confirmRow], ephemeral: true });
    } else if (interaction.customId.startsWith('confirm_')) {
        const userId = interaction.customId.split('_')[1];
        const logChannel = await client.channels.fetch(process.env.PAYMENTS_LOGS_CHANNEL);
        const logs = await logChannel.messages.fetch({ limit: 100 });
        const target = logs.find(m => { try { return JSON.parse(m.content).userId == userId; } catch { return false; } });

        if (target) await target.delete();
        await interaction.update({ content: "✅ Payé et supprimé.", components: [] });
        updatePublicTable();
